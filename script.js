...
let sessions = {}; // session_id -> messages
let current = null;
...
function renderHistory(){
  ...
  Object.keys(sessions).reverse().forEach(id => {
    ...
    div.innerHTML = `<div class="h-main"><div style="width:36px;height:36px;border-radius:8px;background:linear-gradient(135deg,#7c3aed,#06b6d4);display:flex;align-items:center;justify-content:center">💬</div><div><div class="h-title">${title}</div><div class="h-sub">${msgs.length? 'Messages: '+msgs.length : 'Empty'}</div></div></div><div style="display:flex;gap:8px"><button class="btn" data-archive="${id}">📦</button><button class="btn" data-delete="${id}">🗑️</button></div>`;
...
async function sendMessage(){
  ...
  try{
    const res = await fetch(window.location.origin + '/chat', {
      ...
      body: JSON.stringify({session_id: current, user_message: text})
    });
    if(!res.ok) throw new Error('Server error '+res.status);
    const data = await res.json();
    typing.remove();
    sessions[current].push({role:'bot', text: data.reply});
    saveLocal();
    renderChat();
    renderHistory();
  }catch(err){
    typing.remove();
    sessions[current].push({role:'bot', text:'❌ Gabim: '+err.message});
    saveLocal();
    renderChat();
  }
}
...
newBtn.addEventListener('click', ()=>{ current = createSession(); renderChat(); });
archiveAll.addEventListener('click', ()=>{ alert('Archive demo — use server API to archive.'); });
